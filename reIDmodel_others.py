...
class baseline_IR(nn.Module):
    def __init__(self, class_num = 10, low_dim = 512, droprate=0.5, arch='resnet50'):
        super(baseline_IR, self).__init__()
        if arch == 'resnet18':
        ...
        self.feature = FeatureBlock(pool_dim, low_dim, dropout=droprate)
        self.classifier = ClassBlock(low_dim, class_num, dropout=droprate)
    ...
    def forward(self, x1, x2, modal=0):
        ...
        if self.training:
            return out, self.l2norm(y), self.l2norm(y)
        else:
            return self.l2norm(x), self.l2norm(y), 0
